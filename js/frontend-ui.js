class FrontendUI {
  ...
  showProductDetails(product) {
    ...
    const seriesDisplayName = frontendCore.state.seriesNameMap[product.seriesId] || product.seriesId;
  ...
  // 获取产品翻译
  getProductTranslation(value, type) {
    const lang = frontendCore.state.currentLang;
  ...
  renderProducts() {
    ...
    const startIndex = (frontendCore.state.currentPage - 1) * frontendCore.config.itemsPerPage;
    const endIndex = startIndex + frontendCore.config.itemsPerPage;
    const paginatedProducts = frontendCore.state.products.slice(startIndex, endIndex);
    
    if (paginatedProducts.length === 0) {
      productGrid.innerHTML = '<div class="no-products">暂无产品</div>';
      return;
    }
      ...
      html += `
        <div class="product-card" data-product-id="${product.id}">
          <div class="product-image-container">
            <img src="${product.images && product.images.length > 0 ? product.images[0] : 'https://via.placeholder.com/300'}" 
                 alt="${translatedName}" 
                 class="product-image" 
                 onclick="frontendUI.openLightbox('${product.images && product.images.length > 0 ? product.images[0] : 'https://via.placeholder.com/300'}')">
          </div>
          <div class="product-info">
            <div class="product-series">${seriesDisplayName}</div>
            <h3 class="product-name">${translatedName}</h3>
            <button class="view-details-btn" onclick="frontendUI.showProductDetails(${JSON.stringify(product).replace(/"/g, '&quot;')})"><span class="btn-text">查看详情</span></button>
          </div>
        </div>
      `;
    });
...
const frontendUI = new FrontendUI();
