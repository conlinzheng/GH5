class UIManager {
  ...
  updateFormLabels() {
    ...
    const formLabels = contactForm.formLabels || {};
    
    const labels = [
      { id: 'label-name', key: 'name' },
      { id: 'label-email', key: 'email' },
      { id: 'label-message', key: 'message' },
      { id: 'submit-btn', key: 'submit' }
    ];
    
    labels.forEach(label => {
      const el = document.getElementById(label.id);
      if (el) {
        if (label.id === 'submit-btn') {
          el.textContent = formLabels[label.key]?.[lang] || formLabels[label.key]?.zh || '提交';
        } else {
          el.textContent = formLabels[label.key]?.[lang] || formLabels[label.key]?.zh || label.key;
        }
      }
    });
  }
  ...
  getFormspreeUrl() {
    const config = this.frontend.state.siteConfig;
    const contactForm = config.contactForm || {};
    return contactForm.formspreeId ? `https://formspree.io/f/${contactForm.formspreeId}` : '';
  }
}
