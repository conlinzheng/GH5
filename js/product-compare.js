    ...
    renderCompareBar() {
        ...
        for (let i = 0; i < this.maxCompare; i++) {
            ...
            if (this.compareList[i]) {
                ...
                slot.innerHTML = `
                    <img src="https://raw.githubusercontent.com/conlinzheng/GH5/main/产品图/${product.seriesId}/${product.id}" alt="${product.name?.zh || ''}">
        ...
        let html = `
            <table class="compare-table">
                ...
                <tbody>
                    <tr>
                        <td>图片</td>
                        ${products.map(p => `<td><img src="https://raw.githubusercontent.com/conlinzheng/GH5/main/产品图/${p.seriesId}/${p.id}" alt="${p.name?.zh}" style="width:120px;height:120px;object-fit:contain;"></td>`).join('')}
                    </tr>
                    <tr>
                        <td>价格</td>
